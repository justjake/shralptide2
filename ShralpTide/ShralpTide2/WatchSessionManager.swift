...
class WCSession {
    ...
    var delegate: WatchSessionManager?
    var isReachable: Bool?
    func activate() {}
    func sendMessage(_: [String: Any], replyHandler _: Any?, errorHandler _: (Error) -> Void) {}
}
...
class WatchSessionManager: NSObject {
    ...
    private let session: WCSession? = nil
        ...
        if let request = message["request"] as! String? {
            if request == "provision" {
                if let state = appState {
                    var settings = state.config.settingsDict as! [String: Any]
    ...
    func sendStateUpdate() {
        if let state = appState {
            if session?.isReachable ?? false {
                ...
                session?.sendMessage(settings, replyHandler: nil, errorHandler: { _ in
